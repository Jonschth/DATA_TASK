...
County_2_data_reformatted <-
  County_2_data %>% 
    ...
    knitr::kable(head(County_2_data_reformatted,10), format = "pipe")
```
...
Experiment_data_merged <-
  Experiment_data %>% 
    ...
    knitr::kable(head(Experiment_data_merged, 10),format = "pipe")
```
...
### The table shows that the answer to research question is no irrespective of the filter applied. It also shows that the answers are the same for the total sample and females. Where is gets more interesting is when we start filtering for race and counties. 

### For the county 1 it looks as if sending info message or norms message does not singificantly influence applicants. 
